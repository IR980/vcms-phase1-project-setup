...
interface ExpiringDocumentsListProps {
  /**
   * Real documents returned by the compliance API.
   */
  ...
  /**
   * Optional callback to open document details.
   */
  onView?: (document: ComplianceDocumentDto) => void;

  /**
   * Number of days used by backend for the
   * expiring-soon query.
   *
   * This is only displayed as information.
   * The actual filtering is done by backend.
   */
  days?: number;
...
const ExpiringDocumentsList = ({
  ...
  if (isLoading) {
    return (
      <ComplianceLoading
        message="Loading expiring documents..."
  ...
  if (!documents || documents.length === 0) {
    return (
      <ComplianceEmptyState
        ...
        message={`There are no documents expiring within the next ${days} days.`}
        className={className}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3 border-b border-yellow-100 bg-yellow-50/50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <span className="inline-flex w-fit items-center rounded-full border border-yellow-200 bg-white px-2.5 py-1 text-xs font-semibold text-yellow-700">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-100">
            {documents.map((document) => (
              <tr
                key={document._id}
                className="transition hover:bg-yellow-50/30"
              >
                {/* ======================================== */}
                {/* DOCUMENT */}
                {/* ======================================== */}
